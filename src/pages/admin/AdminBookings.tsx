  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Lab Sessions</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {filteredBookings.length === 0 ? (
            <div className="col-span-full p-12 text-center rounded-3xl bg-white border border-slate-200 shadow-xs">
              ...
              <h3 className="font-heading font-bold text-slate-900 text-sm">No lab or session slots scheduled</h3>
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">Click &quot;New Booking&quot; above to publish lab booking slots for students.</p>
            </div>
          ) : (
              ...
              {filteredBookings.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-xs text-slate-400">
                    No lab or session slots scheduled yet.
                  </td>
                </tr>
              ) : (
                filteredBookings.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="py-3.5 px-4 font-medium text-slate-800">{b.subjectName}</td>
